(async () => {
	const labelName = "{{args.name}}".trim();
	...
	let targetPrefix = "#label/" + encodeURIComponent(labelName);
	if (
		["social", "promotions", "updates", "forums", "purchases"].includes(lower)
	) {
		targetPrefix = "#category/" + lower;
	} else if (["important", "imp"].includes(lower)) {
		targetPrefix = "#imp";
	}
