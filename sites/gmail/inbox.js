(async () => {
	...
	const expectedHash = page > 1 ? `#inbox/p${page}` : "#inbox";
	if (
	...
	const startIndex =
		page > 1 && window.location.hash === "#inbox" ? (page - 1) * limit : 0;
	const targetSlice = filteredRows.slice(startIndex, startIndex + limit);
