(() => {
  const out = [];
  const seen = new Set();
  document.querySelectorAll('a[href*="/web/reader/"], a[href*="bookDetail"], .ranking_content_book, .bookInfo, li').forEach((row) => {
    if (out.length >= {{args.limit}}) return;
    const a = row.matches?.('a[href]') ? row : row.querySelector?.('a[href]');
    const box = row.matches?.('a[href]') ? (row.closest('li, .ranking_content_book, .bookInfo, div') || row) : row;
